...
The uncertainty in the continuous life-history parameters was deal with in two ways: a discrete uncertainty grid and stochastic sampling of the life-history parameters from a continuous distribution.

## Discrete Uncertainty Grid

A discrete OM Uncertainty Grid was developed assuming three levels for each parameter. This approach follows the methodology used in other ICCAT MSE processes, and results in 9 individual operating models, each with discrete values from the factorial combinations of the axes and levels of uncertainty (Table \@ref(tab:LH-grid)).
...
The *RCM* *Base Mean* OM was conditioned using the mean value of the stochastic life-history parameters (see Section \@ref(uncertainties-in-life-history-parameters)); i.e., all life-history parameters were identical across the individual simulations.
